from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QPushButton, QLineEdit, QLabel, QTextEdit, QCheckBox, QFileDialog, QSlider
...
class ModulationDialog(QDialog):
    def __init__(self):
        ...
        bitrate_label = QLabel("Baud Rate:", font=font)  
        self.bit_rate_input = QLineEdit(self)  
        self.bit_rate_input.setPlaceholderText("Enter bit rate (Bd)")
        ...
        try:
            ...
            if modulator.save_signal == True:
                ...
                if (not filename) or (" " in filename):
                    ...
                    modulator.save(f'user_file__{message}__{int(modulator.carrier_freq/1000)}kHz_{int(bit_rate/1000)}kBd_{save_mode}.wav', modulated_sig) 
                else:
                    self.display_message(f"File saved with named mode: {self.file_name_input.text()}")
                    modulator.save(f'user_file__{self.file_name_input.text()}__{int(modulator.carrier_freq/1000)}kHz_{int(bit_rate/1000)}kBd_{save_mode}.wav', modulated_sig)
            # Generate the figure
