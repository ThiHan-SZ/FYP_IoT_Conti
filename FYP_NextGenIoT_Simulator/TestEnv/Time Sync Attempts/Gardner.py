...
class Demodulator:
    ...
    def gardner_timing_recovery(received_signal, samples_per_symbol, initial_offset=0):
        """
...
def main():
    ...
    gardner_signal = demodulator.gardner_timing_recovery(delaymodulatedsignal)

    demodulated_signal = demodulator.demodulate(gardner_signal)
    text = demodulator.demapping(demodulated_signal)
    demodulator.demod_and_plot(gardner_signal)
    demodulator.fig.suptitle("Received Signal")
    print(f'Received Message : {text}')
