...
class SequanSocket:
    ...
    def isEnable(self):
        return self.__sendCmdAndCheck(self.__CMD_dict["CHECK_MODULE"], "+CEREG: 4,1", "+CEREG: 2,1")
    ...
    def getMoni(self):
        result_string = ""
        if self.__serialPort is not None:
            command = "AT+SQNMONI=0\r\n"
            self.__serialPort.write(command.encode())
            timeUtil.milliSleep(200)
            result_lines = self.__serialPort.readlines()
            # print("line:",result_lines)
            for line in result_lines:
                if "+SQNMONI:" in line.decode():
                    logging.debug(line)
                    line_decode = line.decode()
                    result_string = line_decode
        return result_string

    def getMoniAndResumeSocket(self):
        IS_EXIT = False
        self.exitTransmission()
        while not self.checkATfunction() and not IS_EXIT:
